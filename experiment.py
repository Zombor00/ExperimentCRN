...
predict_counterfactuals = True
torch.manual_seed(42)


def get_data_simple_cancer(size, counterfactual=False, treatment_effect=False):
    ...
    # We normalize the data (we do not care about saving
    # mean and std to get real data back)
    ...
    for i in range(len(Y1)):
        dataset.append([torch.tensor((X0[i], A0[i])),
                        torch.tensor((Y1[i], A0[i]))])
...
def get_data_simple_cancer_counterfactual(factual_data):
    ...
    for elem1, elem2 in factual_data:
        dataset.append([torch.tensor((elem1[0], (elem1[1]+1) % 2)),
                        torch.tensor((elem2[0], (elem2[1]+1) % 2))])
    return dataset
...
def train_loop(dataset, model, loss_fn, optimizer, balance):
    ...
    for (X, y) in dataset:
        ...
        if (((pred[1] > 0.5) and y[1] == 1) or
           ((pred[1] <= 0.5) and y[1] == 0)):
    ...
    print(f"Train Error: \n RMSE loss: {y_loss:>8f} \
        Accuracy prediction A: {accuracy_prediction_a}% \n")
...
def test_A(dataset, model, model_a):
    ...
    print(f"Accuracy prediction A no balance: \
        {accuracy_prediction_a * 100}% \n")
    return accuracy_prediction_a


dataset_train = get_data_simple_cancer(NUM_TRAIN)
dataset_val = get_data_simple_cancer(NUM_VAL)
if (predict_counterfactuals):
    dataset_test = get_data_simple_cancer(NUM_TEST, counterfactual=True)
else:
    dataset_test = get_data_simple_cancer(NUM_TEST, counterfactual=False)
